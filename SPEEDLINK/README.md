...
## Правильный первый сценарий для нового пользователя

Первый запуск — это не сразу создание профиля и не сразу фидер. Сначала нужно выбрать стабильный USB-порт.

1. Подключить контроллер к первому кандидату USB-порта.
...
3. В `joy_diag.py` выбрать устройство и проверить, что видны оси/кнопки/POV.
4. Для каждого USB-порта-кандидата выполнить тест стабильности подключения:
   - вписать метку порта, например `front1`, `rear1`, `rear2`;
   - нажать **«Снять раунд»**;
   - переподключить контроллер в следующий порт;
   - снова нажать **«Снять раунд»**;
   - сделать по 2–3 раунда на порт;
   - нажать **«Рекомендация»**.
5. Из всех портов оставить только стабильные по центру/разбросу.
6. На выбранных стабильных портах сделать **«Замер дрожания (60с, все оси)»**.
7. Нажать **«Рекомендация по дрожанию»** и выбрать 1–2 лучших порта.
8. Только на финальном выбранном порту сделать профиль:
   - **«Замерить покой»**;
   - **«Калибровка краёв»**, если физический ход не достаёт до краёв диапазона;
   - **«Сохр. JSON»** как `joydiag_profile_final.json`.
9. Проверить боевой фидер:

```powershell
python vjoy_feeder.py
```

10. Проверить vJoy в `joy.cpl`. Если всё корректно — настраивать HidHide и автозапуск.

Коротко:

```text
порты -> стабильность центра -> дрожание 60с -> финальный порт -> покой/края -> профиль -> vJoy feeder
```
